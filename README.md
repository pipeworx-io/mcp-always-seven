...
`tools/list` at `https://gateway.pipeworx.io/always-seven/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/always_seven_generate \
  -H 'Content-Type: application/json' \
  -d '{"question":"What is my lucky number?"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/always_seven_generate`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "always-seven": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-always-seven"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-always-seven
```
